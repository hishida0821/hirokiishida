...
async function getProductsPost(id: string): Promise<Props> {
...
export default async function ProductsPostPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const post = await getProductsPost(id);
...
export async function generateStaticParams() {
  const contentIds = await client.getAllContentIds({ endpoint: 'products' });
